...
context.showImpactFeedback=()=>{};
const splash=source.match(/function splashDamage\([\s\S]*?\n}\n/);
assert.ok(splash,'splash falloff exists');
vm.runInNewContext(`${splash[0]}\n${detonate[0]}`,context);
const blast={owner:'me',mesh:{position:new Vector3(0,0,0),geometry:{dispose(){}},material:{dispose(){}}}};
context.detonatePlasma(blast,null);
assert.deepEqual(hits,[['near',26]],'scenery explosion damage tapers with distance');
